...
CUSTOM_CSS = f"""
<style>
/* Keep default white background and black text */

/* Hero title bar (blue only here) */
.hero-title {{
...
/* Download button styled in Bell blue */
...
# ===========================================================
#  Analyzer: load data, extract text, call AI
# ===========================================================
class ReportAnalyzer:
    ...
    def __init__(self, reports_config: Dict[str, Dict[str, Any]]):
        ...
        self.reports_config = reports_config  # already includes file, file_type, description, focus_areas, sample_questions, prompt_file
...
# ===========================================================
#  UI — sidebar (report info) + main (hero, download, samples, chat)
# ===========================================================
def main():
    # Load full report config from external file
    ...
    # Sidebar: report selection + info + focus areas
    with st.sidebar:
        st.header("📋 Report")
    ...
    st.download_button(
        label=f"📥 Download {selected_report.split()[0]} Report",
        data=file_bytes,
    ...
    user_input = st.chat_input(f"Ask about {selected_report}...")

    # Inject suggested question into chat
    ...
    if user_input and df is not None:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Analyzing..."):
                ...
                # Load unified prompt from prompt_file with fallback
                unified_prompt = read_prompt_text(report_cfg.get("prompt_file"))

                question_context = question_config.get("context", "") if isinstance(question_config, dict) else ""
